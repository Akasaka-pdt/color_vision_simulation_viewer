...
def simulate_deficiency(rgb, deficiency_type):
    ...
    if deficiency_type == 'protanopia':
        M = np.array([[0, 1.208, -0.208], [0, 1, 0], [0, 0, 1]])
    elif deficiency_type == 'deuteranopia':
        M = np.array([[1, 0, 0], [0.8278, 0, 0.1722], [0, 0, 1]])
    elif deficiency_type == 'tritanopia':
        M = np.array([[1, 0, 0], [0, 1, 0], [-0.5254, 1.5254, 0]])
    else:
        return rgb
...
def main():
    ...
    st.html("""
    <style>
        h1 { text-align: center; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; font-size: 1.8vw; color: #C00000; }
    </style>
    ...
    if st.sidebar.button("Process PDF Files", disabled=not uploaded_files):
        ...
        st.html("""
        <style>
            .display_1 { top: 0; left: 0; }
            .table { color: #000000; background-color: #FBF4E5; text-align: center; border: 2px #360000 solid; width: 100%; height: 200px; margin: auto; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; border-collapse: collapse; }
            td{ border: 1px #360000 solid; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; font-size: 1.25vw; vertical-align: middle; line-height: 2.0; }
